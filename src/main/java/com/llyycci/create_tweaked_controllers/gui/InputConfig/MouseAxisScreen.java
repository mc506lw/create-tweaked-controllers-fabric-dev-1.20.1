...
public class MouseAxisScreen extends GenericInputScreen {
    ...
    protected void renderWindow(GuiGraphics graphics, int x, int y, float partialTicks) {
        ...
        graphics.drawString(font, Create_Tweaked_Controllers.translateDirect("gui_config_lower"), width / 2 - boundsTextWidth, height / 2 - 40, 0xaaaaaa);
        graphics.drawString(font, Create_Tweaked_Controllers.translateDirect("gui_config_upper"),width / 2 - boundsTextWidth, height / 2 - 15, 0xaaaaaa);
        graphics.drawString(font, Create_Tweaked_Controllers.translateDirect("gui_input_axis"), width / 2 - textwidth, height - 105, 0xaaaaaa);
    ...
    protected void Populate() {
        ...
        bounds[0] = new EditBox(font, width / 2, height/2 - 45, 90, 20, Create_Tweaked_Controllers.translateDirect("gui_config_lower"));
        bounds[0].setValue(GetSafeFloatString(source.minBound));
        bounds[1] = new EditBox(font, width / 2, height/2 - 20, 90, 20, Create_Tweaked_Controllers.translateDirect("gui_config_upper"));
        bounds[1].setValue(GetSafeFloatString(source.maxBound));
        addRenderableWidgets(bounds);
        valueRender = new EditBox(font, width / 2, height - 110, 50, 20, Create_Tweaked_Controllers.translateDirect("gui_input_axis"));
        ...
        isYBox = new Checkbox(width / 2 - 60, height/2 - 95, 100, 20,
            Create_Tweaked_Controllers.translateDirect("gui_config_isyaxis"), source.isYAxis);
        addRenderableWidget(isYBox);
        useVelBox = new Checkbox(width / 2 - 60, height/2 - 70, 100, 20,
            Create_Tweaked_Controllers.translateDirect("gui_config_usevelocity"), source.useVelocity);
        addRenderableWidget(useVelBox);
        boundsTextWidth = Math.max(font.width(Create_Tweaked_Controllers.translateDirect("gui_config_lower")), font.width(Create_Tweaked_Controllers.translateDirect("gui_config_upper")));
        boundsTextWidth += 10;
    }
