...
public class TweakedLinkedControllerScreen extends AbstractSimiContainerScreen<TweakedLinkedControllerMenu> {
	...
	public TweakedLinkedControllerScreen(TweakedLinkedControllerMenu menu, Inventory inv, Component title) {
		...
		this.background0 = ModGuiTextures.TWEAKED_LINKED_CONTROLLER_0;//设置背景图片
	...
	protected void init() {
		...
		refreshButton.setToolTip(Create_Tweaked_Controllers.translateDirect("gui_button_refresh"));
		...
		firstTabButton.setToolTip(Create_Tweaked_Controllers.translateDirect("gui_tab_button"));
		secondTabButton = new IconButton(x + 42, y + background0.height - 27, ModIcons.I_AXES);
		secondTabButton.withCallback(() -> {
			this.isSecondPage = true;
			menu.SetPage(this.isSecondPage);
		});
		secondTabButton.setToolTip(Create_Tweaked_Controllers.translateDirect("gui_tab_axis"));
	...
	private List<Component> addToTooltip(List<Component> list, int slot)
	{
		...
		if (slot >= 30)
		{
			list.add(Create_Tweaked_Controllers.translateDirect("tweaked_linked_controller.frequency_slot_" + ((slot % 2) + 1),
