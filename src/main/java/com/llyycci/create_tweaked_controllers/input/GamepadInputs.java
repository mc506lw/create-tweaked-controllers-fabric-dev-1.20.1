...
public class GamepadInputs {
    ...
    public static final Component GetButtonName(int index) {
        if (index < 4) {
            switch (ModClientConfig.CONTROLLER_LAYOUT_TYPE.get()) {
                case NINTENDO:
                    return Create_Tweaked_Controllers.translateDirect("gui_gamepad_button_" + (index ^ 0x1)); // Swap X/Y and A/B butons for Nintendo controllers

                case PLAYSTATION:
                    return Create_Tweaked_Controllers.translateDirect("gui_gamepad_button_playstation_" + index); // Use PlayStation controller symbols

                default:
                    return Create_Tweaked_Controllers.translateDirect("gui_gamepad_button_" + index);
            }
        }
        return Create_Tweaked_Controllers.translateDirect("gui_gamepad_button_"+index);
    }
    ...
    public static final Component GetAxisName(int index) {
        return Create_Tweaked_Controllers.translateDirect("gui_gamepad_axis_"+index);
    }
}
